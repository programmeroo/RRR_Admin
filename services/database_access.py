...
def set_server(use_server="local"):
    ...
    if use_server == "local":
        ...
        SERVER_DB = REMOTE_DB
    print(f"Using SERVER_DB: {SERVER_DB}")
...
def delete_archive(table):
    return _request("DELETE", f"archive/{table}")
...
def get_flyer_viewers():
    return _request("GET", "flyer_viewers")
...
def purge_api_log(before):
    return _request("DELETE", "api_log", params={"before": before})
